-- =============================================================================
-- Migration: Gestion du service Cours Collectifs (public.service_settings)
-- Date: 2026-09-24
-- Description:
--   1. Insertion idempotente du service 'collective' dans public.service_settings
--   2. Mise à jour de create_trial_booking pour vérifier l'état du service Cours Collectifs
-- =============================================================================

-- 1. Insertion du service Cours collectifs s'il n'existe pas déjà
INSERT INTO public.service_settings (service_key, service_name, description, is_active)
VALUES (
  'collective',
  'Cours collectifs',
  'Séances collectives de Boxe Anglaise, Kick Boxing, Muay Thaï et Lady Striking',
  true
)
ON CONFLICT (service_key) DO NOTHING;

-- 2. Redéfinition sécurisée de create_trial_booking pour vérifier is_service_active('collective') et is_service_active('small_group')
...
BEGIN
  ...
  -- 4.ter Contrôle de l'état d'activation du service Cours Collectifs
  IF v_session.type = 'collective' AND NOT public.is_service_active('collective') THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'SERVICE_UNAVAILABLE',
      'message', 'Le service Cours Collectifs est actuellement désactivé et indisponible à la réservation.'
    );
  END IF;
  ...
  -- 7. Calcul d'occupation sous verrou pour Small Group
  ...
  RETURN jsonb_build_object(
    ...
    'message', 'Votre réservation de cours d''essai est confirmée.'
  );
END;
$$;
